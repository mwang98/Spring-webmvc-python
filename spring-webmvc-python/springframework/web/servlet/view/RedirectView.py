...
class RedirectView(SmartView, AbstractUrlBasedView):
    ...
    def set_expand_uri_template_variables(self, expandUriTemplateVariables: bool) -> None:
    ...
    def create_target_url(self, model: dict, request) -> str:
        ...
        if self.expandUriTemplateVariables and targetUrl:
            ...
            targetUrl = self.replace_uri_template_variables(str(targetUrl), model, variables, enc)
    ...
    def append_query_properties(self, targetUrl: str, model: dict, encodingScheme: str) -> None:
        fragment: str = None
        if '#' in targetUrl:
            anchorIndex = targetUrl.index('#')
            fragment = targetUrl[anchorIndex:]
        ...
        for key, rawValue in self.query_properties(model).items():
            ...
            for value in values:
                ...
                encodedValue: str = "" if value is None else self.url_encode(str(value), encodingScheme)
    ...
    def query_properties(self, model: dict) -> dict:
        result = {key: value for key, value in model.items() if self.is_eligible_property(key, value)}
        return result
    ...
    def is_eligible_value(slef, value: object = None) -> bool:
        # TODO: BeanUtils
        return ((value is not None) and (BeanUtils.isSimpleValueType(value.__class__)))

    def url_encode(self, input: str, encodingScheme: str) -> str:
        # TODO: URLEncoder
        return quote_plus(input, encoding=encodingScheme)

    def update_target_url(self, targetUrl: str, model: dict, request, response):
    ...
    def is_remote_host(self, targetUrl: str) -> bool:
        ...
        targetHost: str = UriComponentsBuilder.fromUriString(targetUrl).build().getHost()
        if not targetHost:
            return False
    ...
    def get_http_11_status_code(self, request, response, targetUrl: str) -> HttpStatus:
        if self.statusCode is not None:
