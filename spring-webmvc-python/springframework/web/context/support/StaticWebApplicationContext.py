from springframework.context.support.StaticApplicationContext import StaticApplicationContext
from springframework.utils.mock.inst import ServletContextAwareProcessor, \
    ServletContextAware, ServletConfigAware, WebApplicationContextUtils, \
...
class StaticWebApplicationContext(StaticApplicationContext):
    ...
    def set_namespace(self, namespace: str):
        ...
        if namespace is None:
            self.set_display_name(f"WebApplicationContext for namespace '{namespace}'")

    def get_namespace(self):
        return self.namespace

    def set_config_location(self, configLocation: str):
        raise Exception("StaticWebApplicationContext does not support config locations")
    ...
    def post_process_bean_factory(self, beanFactory):
        beanFactory.addBeanPostProcessor(ServletContextAwareProcessor(self.servletContext, self.servletConfig))
        beanFactory.ignoreDependencyInterface(ServletContextAware)
        beanFactory.ignoreDependencyInterface(ServletConfigAware)

        WebApplicationContextUtils.registerWebApplicationScopes(beanFactory, self.servletContext)
        WebApplicationContextUtils.registerEnvironmentBeans(beanFactory, self.servletContext, self.servletConfig)
