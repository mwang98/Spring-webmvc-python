...
class MockHttpServletResponse:
    ...
    def update_content_type_property_and_header(self) -> None:
        if self.contentType is not None:
            ...
            if self.charset and self.CHARSET_PREFIX not in self.contentType.lower():
                value = value + ';' + self.CHARSET_PREFIX + self.characterEncoding
    ...
    def get_output_stream(self) -> ServletOutputStream:
        assert self.outputStreamAccessAllowed, "OutputStream access not allowed"
    ...
    public PrintWriter getWriter() throws UnsupportedEncodingException {
        ...
        if (self.writer == null) {
            Writer targetWriter = (self.characterEncoding != null ?
                    new OutputStreamWriter(self.content, self.characterEncoding) :
    ...
    def set_content_length(self, contentLength: int) -> None:
        ...
        self.do_add_header_value(HttpHeaders.CONTENT_LENGTH, contentLength, True)
    ...
    def reset_buffer(self) -> None:
        assert not self.is_committed(), "Cannot reset buffer - response is already committed"
        self.content.reset()
    ...
    def set_locale(self, locale: Locale) -> None:
        self.locale = locale
        self.do_add_header_value(HttpHeaders.CONTENT_LANGUAGE, locale.toLanguageTag(), True)
    ...
    def add_cookie(self, cookie: Cookie) -> None:
        ...
        self.do_add_header_value(HttpHeaders.SET_COOKIE, self.get_cookie_header(cookie), False);

    def get_cookie_header(self, cookie: Cookie) -> str:
        buf = ''
        buf += cookie.get_name() + '='
        if cookie.get_value() == None:
            buf += ""
        else:
        ...
        if maxAge >= 0:
            ...
            if expires != None:
                ...
                headers = HttpHeaders()
                if maxAge > 0:
                    headers.setExpires(int(round(time.time() * 1000)) + 1000 * maxAge)
                else:
    ...
    def get_date_header(self, name: str) -> int:
        ...
        try:
            ...
            raise ValueError("Value for header '" + name + "' is not a valid Date: " + headerValue)
    ...
    def get_included_url(self) -> str:
        ...
        assert count <= 1, "More than 1 URL included - check getIncludedUrls instead: "  # + self.includedUrls);
    ...
    private class ResponsePrintWriter extends PrintWriter {
        ...
        @Override
        public void flush(self) -> :
            super.flush();
            setCommitted(true);
        }

        @Override
        public void close(self) -> :
            super.flush();
            super.close();
            setCommitted(true);
        }
