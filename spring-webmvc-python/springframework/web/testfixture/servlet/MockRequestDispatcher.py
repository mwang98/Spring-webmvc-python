...
class MockRequestDispatcher():
    ...
    def forward(self, request, response) -> None:
        ...
        assert not response.is_committed(), "Cannot perform forward - response is already committed"
        self.get_mock_http_servlet_response(response).set_forwarded_url(self.resource)

    def include(self, request, response) -> None:
        assert request is not None, "Request must not be null"
        assert response is not None, "Response must not be null"
        self.get_mock_http_servlet_response(response).add_included_url(self.resource)
